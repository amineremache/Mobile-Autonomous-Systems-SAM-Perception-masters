%% NOTE : Je n'utilise MATLAB que depuis 2 mois, 
% je m'excuse d'avance pour la qualité du code et si ce n'est pas optimal
...
best_rmse = 0;
for sample_size=20:20  % Le but de cette boucle était pour moi d'essayer différentes tailles de combinaiaons de points de liaisons
for iteration=1:1 % le but de cette boucle était de donner un nombre d'itérations pour chaque taille.
%links(iteration,:) = [];
%test = reshape([35 479 87 123 275 21 449 13 51 163 129 211 184 241 357 70 77 234 17 410],20,1);
%links = original_links(test,:);

links = original_links(randsample(length(original_links),sample_size),:);  % pour essayer les combinaisons de points de liaisons, changer le length(original_links) en length(original)

%% Question 8

% Pour cette question vous trouverez parfois du code commenté pour
% implémenter les matrices séparémment, j'ai essayé les 2 méthodes, et
% j'ai préféré continuer la suite avec une seule matrice A et B.
...
A=ones(m,2*(p+1));
for i=1:m
    A(i,2)=vertex(links(i,1),2); % chercher les coordonnées X des iterations de liaison dans la vertex
    A(i,2+(p+1))=vertex(links(i,1),3); % chercher les coordonnées Y des iterations de liaison dans la vertex
end
A(:,3:4)=[A(:,2).^(p-1) A(:,2).^(p)];
A(:,7:8)=[A(:,2+(p+1)).^(p-1) A(:,2+(p+1)).^(p)]; 
B=links(:,2:3);
...
newA = ones(length(vertex),2*(p+1));
newA(:,2)= vertex(:,2);
newA(:,3:4)= [newA(:,2).^(p-1) newA(:,2).^(p)];
newA(:,2+(p+1))= vertex(:,3);
newA(:,7:8)= [newA(:,2+(p+1)).^(p-1) newA(:,2+(p+1)).^(p)];
...
% NOTE : veuillez commenter/décommenter la variable qui convient au
% test que vous voulez effectuer (5-fold ou leave-one-out (LOO))

m = size(original_links,1);
%indices = [1:m/6:5*m/6]; % Pour 5-fold
indices = [1:1:m]; % Pour leave-one-out
best_rmse_CV = 0;

for train_indice=indices
    %test_set = original_links(train_indice:train_indice+(m/6)-1,:); % Pour 5-fold
    test_set = original_links(train_indice,:); % Pour LOO
    ...
    A=ones(m,2*(p+1));
    for i=1:m
        A(i,2)=vertex(train_set(i,1),2); % chercher les coordonnées X des iterations de liaison dans la vertex
        A(i,2+(p+1))=vertex(train_set(i,1),3); % chercher les coordonnées Y des iterations de liaison dans la vertex
    end
    A(:,3:4)=[A(:,2).^(p-1) A(:,2).^(p)];
    A(:,7:8)=[A(:,2+(p+1)).^(p-1) A(:,2+(p+1)).^(p)]; 
    B=train_set(:,2:3);
    ...
    % utiliser les paramètres éstimés pour créer la matrice A
    
    newA = ones(size(test_set,1),2*(p+1));
    for i=1:size(test_set,1)
        newA(i,2)=vertex(test_set(i,1),2);
        newA(i,2+(p+1))=vertex(test_set(i,1),3);
    end
    newA(:,3:4)= [newA(:,2).^(p-1) newA(:,2).^(p)];
    newA(:,7:8)= [newA(:,2+(p+1)).^(p-1) newA(:,2+(p+1)).^(p)];
    ...
    %fprintf(fid, '\nRMSE fold %d: %f m\n',((train_indice-1)*6/length(original_links))+1,rmse); %Pour 5-fold
    fprintf(fid, '\nRMSE fold %d: %f m\n',train_indice,rmse); %Pour LOO
    fclose(fid);
...
fprintf('\nBest RMSE en Cross Validation %f m\n',best_rmse_CV); % Si on veux chercher quelle combinaison de points voir le fichier log.